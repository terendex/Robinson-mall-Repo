 ...
 * but filters to manager-relevant notifications only.
 */
const ManagerNotifications = () => {
  const { notifications, removeNotification, markAsRead, markAllAsRead } = useContext(NotificationContext);

  // Filter notifications relevant to managers
  const managerNotifications = useMemo(() => {
    const keywords = ['voucher', 'campaign', 'claim', 'transaction', 'customer', 'approval', 'registration', 'store', 'shop'];
  ...
  const unreadCount = managerNotifications.filter(n => !n.is_read).length;
  ...
  const clearAllNotifications = () => {
    managerNotifications.forEach(n => removeNotification(n.id));
  };
